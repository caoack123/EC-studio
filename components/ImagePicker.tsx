...
interface ImagePickerProps {
  // Fixed: Made onImageSelected optional to avoid errors in App.tsx when using multiple=true and only providing onImagesSelected.
...
const ImagePicker: React.FC<ImagePickerProps> = ({ onImageSelected, onImagesSelected, label, className, preview, multiple }) => {
  ...
  const handleFiles = (files: FileList | null) => {
    ...
    } else if (files[0] && onImageSelected) {
      // Fixed: Added check for onImageSelected since it is now optional.
      const reader = new FileReader();
      ...
      >
        {preview ? (
          ...
          <>
            ...
            <p className="text-sm text-slate-600 font-bold">Drag & Drop or Click</p>
            <p className="text-xs text-slate-400 mt-1">Upload {multiple ? 'multiple' : 'an'} image asset</p>
          </>
        )}
      </div>
      <input 
